...
## Purpose
Accelerometer magnitude error variance tracking for state transition decisions. Replaces tilt-based variance with raw accelerometer error `|√(ax²+ay²+az²) - 1.0|` which responds immediately to external forcing without lag from the complementary filter.
