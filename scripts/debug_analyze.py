...
def recompute_frequency(raw_data: List[float], decay_start: int,
    ...
    region = np.array(region, dtype=np.float64)
    region = region - np.mean(region)  # match ESP32 default de-meaning
...
def generate_plot(snapshot: DebugSnapshot, output_file: Optional[str] = None):
    ...
    # Subplot 1: Raw signal with decay region highlighted
    ax0 = axes[0, 0]
    ...
    if snapshot.roll_decay_count > 0:
        ...
        if r1 > len(t):
            r1 = len(t)
        ax0.axvspan(t[r0], t[r1 - 1], alpha=0.15, color="blue", label="Roll decay")
    if snapshot.pitch_decay_count > 0:
        p0 = snapshot.pitch_decay_start
        p1 = p0 + snapshot.pitch_decay_count
        if p1 > len(t):
            p1 = len(t)
        ax0.axvspan(t[p0], t[p1 - 1], alpha=0.15, color="red", label="Pitch decay")
    ...
    # Subplot 2: Welch PSD with peak annotated
    ax1 = axes[0, 1]
    ...
    # Subplot 3: Peak envelope with regression line
    ax2 = axes[1, 0]
    for axis_name, amps, times, py_freq, py_zeta, color, label in [
        ...
        if len(amps) < 2:
            continue
        ax2.semilogy(times, amps, "o-", color=color, markersize=4, linewidth=0.8, label=f"{label} peaks")
        ...
        if len(amps) >= 4 and py_freq > 0 and py_zeta > 0:
            ...
            ax2.semilogy(t_fit, fit_line, "--", color=color, linewidth=1.0,
                        label=f"{label} fit (zeta={py_zeta:.4f})")

    ax2.set_xlabel("Time (s)")
    ax2.set_ylabel("Peak Amplitude")
    ax2.set_title("Peak Envelope + Log-Linear Regression")
    ...
    table_data = [
        ...
        ["Freq Roll (Hz)",
         ...
         f"{py_freq_r:.4f}",
         "YES" if abs(snapshot.esp_freq_roll_hz - py_freq_r) < 0.001 else "NO"],
        ["Zeta Roll",
         f"{snapshot.esp_zeta_roll:.4f}",
         f"{py_zeta_r:.4f}",
         "YES" if abs(snapshot.esp_zeta_roll - py_zeta_r) < 0.001 else "NO"],
        ["Freq Pitch (Hz)",
         f"{snapshot.esp_freq_pitch_hz:.4f}",
         f"{py_freq_p:.4f}",
         "YES" if abs(snapshot.esp_freq_pitch_hz - py_freq_p) < 0.001 else "NO"],
        ["Zeta Pitch",
         f"{snapshot.esp_zeta_pitch:.4f}",
         f"{py_zeta_p:.4f}",
         "YES" if abs(snapshot.esp_zeta_pitch - py_zeta_p) < 0.001 else "NO"],
    ...
    table.scale(1.0, 1.4)
    ax3.set_title("ESP32 vs Python Comparison", fontweight="bold")
