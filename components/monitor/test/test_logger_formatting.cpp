...
monitor::MonitorResult MakeResult() {
    ...
    std::strncpy(result.damping_confidence.data(), "medium", result.damping_confidence.size() - 1U);
...
TEST_CASE("logger parameter CSV formatter fits fixed line buffer", "[logger][format]") {
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(line.buffer.data(), "medium"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "event_type"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "onset_idx"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "peak_gmag"));
}
...
TEST_CASE("logger parameter JSON formatter fits fixed line buffer", "[logger][format]") {
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(line.buffer.data(), "\"damping_confidence\":\"medium\""));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "event_type"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "onset_idx"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "offset_idx"));
    TEST_ASSERT_NULL(std::strstr(line.buffer.data(), "peak_gmag"));
}
